  ...
  <BreezeAuthenticatedLayout>
    ...
    <form @submit.prevent="update">
        ...
        <div  v-if="user.notifications.length">
            <div  v-for="notification in user.notifications" :key="notification.id" class="w-full mx-auto">
                <div class="my-5 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
                    <strong class="font-bold">Dear customer,   </strong>
                    <span v-if="notification.data.parking" class="block sm:inline">{{ notification.data.message}}:  {{notification.data.parking.parking_name}}.
                        <Link href="/parking/notify" method="post" :data="{ parking: notification.data.parking, notification: notification }" class="underline">If this is not your car, then click here</Link>
                    </span>
                    <span v-else class="block sm:inline">{{ notification.data.message}}</span>
                    <span class="absolute top-0 bottom-0 right-0 px-4 py-3">
                        <svg @click="clearNotification(notification)" class="fill-current h-6 w-6 text-red-500" role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><title>Close</title><path d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l2.758-3.15-2.759-3.152a1.2 1.2 0 1 1 1.697-1.697L10 8.183l2.651-3.031a1.2 1.2 0 1 1 1.697 1.697l-2.758 3.152 2.758 3.15a1.2 1.2 0 0 1 0 1.698z"/></svg>
                    </span>
                </div>
            </div>
            <div v-show="user.notifications.length > 0" class="flex flex-row">
                <Button @click="clearAllNotifications()">Clear all notifications</Button>
            </div>
        </div>
        <h1 v-else class="text-blue-500">No new notification</h1>
...
export default {
  ...
  methods: {
      clearAllNotifications(){
          Inertia.post(`/user/${this.user.id}/notifications`)
      },
      clearNotification(notification){
          Inertia.visit(`/user/${this.user.id}/notifications`, {
              method: 'post',
              data: {
